...
/*Xbj Bins {{{*/
const double Full_Xbj_Bin[50] = {0.78,0.82,0.86,0.90,0.94,0.98,1.02,1.06,1.10,1.14,1.18,1.22,1.26,1.30,1.34,1.38,1.42,1.46,1.50,1.54,1.58,1.62,1.68,1.74,1.80,1.86,1.92,1.98,2.04,2.12,2.20,2.28,2.36,2.44,2.52,2.60,2.70,2.80,2.90,3.00,3.10};

const int Kin31_Bin = 20;
const double Kin31_Xbj_Bin[20] = {1.22,1.26,1.30,1.34,1.38,1.42,1.46,1.50,1.54,1.58,1.62,1.68,1.74,1.80,1.86,1.92,1.98,2.04,2.12,2.20};

const int Kin32_Bin = 21;
const double Kin32_Xbj_Bin[21] = {1.58,1.62,1.68,1.74,1.80,1.86,1.92,1.98,2.04,2.12,2.20,2.28,2.36,2.44,2.52,2.60,2.70,2.80,2.90,3.00,3.10};

const int Kin41_Bin = 19;
const double Kin41_Xbj_Bin[19] = {1.26,1.30,1.34,1.38,1.42,1.46,1.50,1.54,1.58,1.62,1.68,1.74,1.80,1.86,1.92,1.98,2.04,2.12,2.20};

const int Kin42_Bin = 17;
const double Kin42_Xbj_Bin[17] = {1.80,1.86,1.92,1.98,2.04,2.12,2.20,2.28,2.36,2.44,2.52,2.60,2.70,2.80,2.90,3.00,3.10};

const int Kin50_Bin = 13;
const double Kin50_Xbj_Bin[13] = {0.78,0.82,0.86,0.90,0.94,0.98,1.02,1.06,1.10,1.14,1.18,1.22,1.26};

const int Kin505_Bin = 17;
const double Kin505_Xbj_Bin[17] = {0.98,1.02,1.06,1.10,1.14,1.18,1.22,1.26,1.30,1.34,1.38,1.42,1.46,1.50,1.54,1.58,1.62};

const int Kin51_Bin = 18;
const double Kin51_Xbj_Bin[18] = {1.30,1.34,1.38,1.42,1.46,1.50,1.54,1.58,1.62,1.68,1.74,1.80,1.86,1.92,1.98,2.04,2.12,2.20};

const int Kin52_Bin = 15;
const double Kin52_Xbj_Bin[15] = {1.92,1.98,2.04,2.12,2.20,2.28,2.36,2.44,2.52,2.60,2.70,2.80,2.90,3.00,3.10};

const int Kin65_Bin = 18;
const double Kin65_Xbj_Bin[18] = {1.74,1.80,1.86,1.92,1.98,2.04,2.12,2.20,2.28,2.36,2.44,2.52,2.60,2.70,2.80,2.90,3.00,3.10};
/*}}}*/
...
int CrossSection(const TString& aInputFile,const TString& aLogFile,const TString& aCommentName){
	...
	outlog <<Form("     Ne = %4.3e (%4.2f%%)", Ne->Value, Ne->Sys_Err/Ne->Value*100)<<endl;	
	cerr   <<Form("     Ne = %4.3e (%4.2f%%)", Ne->Value, Ne->Sys_Err/Ne->Value*100)<<endl;	
	...
	cerr << endl<< "@@@@@@ Checking Currents for each run ..." <<endl; 
	outlog  << endl<< "@@@@@@ Checking Currents for each run ..." <<endl; 
    Double_t I_avg = gCheck_Current(RunNoChain);
	cerr << endl<< "@@@@@@ The Average Beam Currents for all runs is: "<<I_avg <<endl; 
	outlog  << endl<< "@@@@@@ The Average Beam Currents for all runs is: "<<I_avg <<endl; 
	...
	TARGET_MASS_A = gGet_Target_Mass_A(Target_Name.Data());
	/*}}}*/
    ...
    /*Add Additional VZ Cut (04/29/2016){{{*/
    double VZ_Cut[2] = {-0.07, 0.07}; //Min and Max
    //     VZ1  VZ2  VZ3 VZ4 VZ5
    // -10...-6...-2...2...6...10   in cm
    if(aCommentName == "VZ1"){ VZ_Cut[0] = -0.10; VZ_Cut[1] =-0.06;} //meter
    if(aCommentName == "VZ2"){ VZ_Cut[0] = -0.06; VZ_Cut[1] =-0.02;} //meter
    if(aCommentName == "VZ3"){ VZ_Cut[0] = -0.02; VZ_Cut[1] = 0.02;} //meter
    if(aCommentName == "VZ4"){ VZ_Cut[0] =  0.02; VZ_Cut[1] = 0.06;} //meter
    if(aCommentName == "VZ5"){ VZ_Cut[0] =  0.06; VZ_Cut[1] = 0.10;} //meter
    /*}}}*/
	...
	if(IsExtTgt){
		...
		Ntg= gCal_Ntg(RunNoChain,Arm.Data(),Target_A, Target_Thickness,Target_Thickness_Stat_Err,NtgChain);
	}
	else{
	...
	Int_t TotalBin = TMath::Nint((AccBin[1]-AccBin[0])/AccBin[2])+1;
    */
	
	vector<double> vXbj_Bin;
	if(Kin=="Kin3.1"||Kin=="Kin3.11"||Kin=="Kin3.12"){
		for(int i=0;i<Kin31_Bin;i++)
			vXbj_Bin.push_back(Kin31_Xbj_Bin[i]);
	}
	else if(Kin=="Kin3.2"||Kin=="Kin3.21"||Kin=="Kin3.22"){
		for(int i=0;i<Kin32_Bin;i++)
			vXbj_Bin.push_back(Kin32_Xbj_Bin[i]);
	}
	else if(Kin=="Kin4.1"||Kin=="Kin4.11"||Kin=="Kin4.12"){
		for(int i=0;i<Kin41_Bin;i++)
			vXbj_Bin.push_back(Kin41_Xbj_Bin[i]);
	}
	else if(Kin=="Kin4.2"||Kin=="Kin4.21"||Kin=="Kin4.22"){
		for(int i=0;i<Kin42_Bin;i++)
			vXbj_Bin.push_back(Kin42_Xbj_Bin[i]);
	}
	else if(Kin=="Kin5.0"||Kin=="Kin5.01"||Kin=="Kin5.02"){
		for(int i=0;i<Kin50_Bin;i++)
			vXbj_Bin.push_back(Kin50_Xbj_Bin[i]);
	}
	else if(Kin=="Kin5.05"){
		for(int i=0;i<Kin505_Bin;i++)
			vXbj_Bin.push_back(Kin505_Xbj_Bin[i]);
	}
	else if(Kin=="Kin5.1"||Kin=="Kin5.11"||Kin=="Kin5.12"){
		for(int i=0;i<Kin51_Bin;i++)
			vXbj_Bin.push_back(Kin51_Xbj_Bin[i]);
	}
	else if(Kin=="Kin5.2"||Kin=="Kin5.21"||Kin=="Kin5.22"){
		for(int i=0;i<Kin52_Bin;i++)
			vXbj_Bin.push_back(Kin52_Xbj_Bin[i]);
	}
	else if(Kin=="Kin6.5"){
		for(int i=0;i<Kin65_Bin;i++)
			vXbj_Bin.push_back(Kin65_Xbj_Bin[i]);
	}
	else{
	   cerr<<"***#LINE459: ERROR***, I don't know the Kinematic name!!!"<<endl;
	   outlog<<"***#LINE459: ERROR***, I don't know the Kinematic name!!!"<<endl;
	   return -111; 
	}

	Int_t TotalBin = vXbj_Bin.size()-1;
    ...
    ///////////////////////////////////
    //Load XS Table here, only once
    ///////////////////////////////////
    Get_XS* XS = new Get_XS();
    XS->Load_Table(Target_Name.Data(), Kin.Data());
    ...
    if(bMakePlot){
        ...
        cerr       << "oO0Oo              --- Making Plots ---               oO0Oo" <<endl;
        cerr       << "oO0Oo oO0Oo oO0Oo oO0Oo oO0Oo oO0Oo oO0Oo oO0Oo oO0Oo oO0Oo oO0Oo oO0Oo "<<endl<<endl;

        TString Path_Out = Form("%s/results/%s/%s/",XGT2_DIR.Data(),Target_Name.Data(),aCommentName.Data());
        Double_t MC_PhaseSpace = SAMC_dOmega*(P_MeV*SAMC_dDp)/Nf_SAMC_Gen_All->Value;
        gMake_Plot(T_Tree, ElectronCuts.Data(), SAMC_Tree, SAMC_AccCut, 
                Target_Name.Data(),IsExtTgt, Arm.Data(), Kin.Data(),Path_Out.Data(),
                P0_Avg, Angle_Avg, Ne->Value, LiveTime->Value, Ntg->Value, I_avg, 
                 MC_PhaseSpace, aCommentName.Data(),XS);
    ...
    if(bCalXS){
        ...
        TString outstring=Form("%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s%*s",
                16,"XS(nb/MeV/sr)",
                16,"XS_Stat_Err",
                16,"XS_Sys_Err",
                16,"XS_MC",
        ...
        if ( Total_Pion_Rej->Value>0&&Total_E_Eff->Value>0&&Ne->Value>0&&Ntg->Value>0){
            ...
            for(int iX = 0; iX<TotalBin; iX++){
                if ( NoCal%5==0 ){
                    outlog<<Form(" +++ Calculating %d Points ...", NoCal)<<endl;
                    cerr  <<Form(" +++ Calculating %d Points ...", NoCal)<<endl;
                }	

                bin_size = fabs(vXbj_Bin[iX+1] - vXbj_Bin[iX]); //the Xbj bin size 
                xbj= (vXbj_Bin[iX+1] + vXbj_Bin[iX])/2.; //We are binning on Xbj so use its bin-center value
                Ep=Ebeam/(1.0+4.0*Ebeam*SinSQ/(2.0*TARGET_MASS_A*xbj)); //use xbj = Q2/2Mp/(Ei-Ep),Q2 = 4Ei*Ep*Sin_SQ 
                qsq = 4.0*Ebeam*Ep*SinSQ;
                nu = qsq/(2.0*TARGET_MASS_A*xbj);
                ...
                Nf_EX = gGet_Nf_EX(RunNoChain,Arm,Kin,Target_Name,xbj, bin_size/2.,ElectronCuts.Data(), VZ_Cut, Bin_Variable.Data(),LiveTimeChain,LiveTimeStatErrChain,LiveTimeSysErrChain,
                ...
                if(bCal_XS){ //Only calculate Y_MC when calculating CS
                    if(Nf_EX->Value>1e-33 && Ne->Value>1e-33)
                        XS_SAMC_SUM = gGet_XS_SAMC_SUM(SAMC_Tree,xbj,bin_size/2.,SAMC_AccCut, VZ_Cut, Arm.Data(), Target_Name.Data(),Kin.Data(),P0_Avg,Angle_Avg,Bin_Variable.Data(),&BinCor,I_avg, XS);
                    else
                ...
                if(bCal_XS){
                    if (  Yield_EX->Value>1.0 ){
                        ...
                        if(Yield_MC->Value>1.0){
                            ...
                            //XS_Born_XEMC = gGet_XS_Born_XEMC(Target_Name.Data(),Kin.Data(),Ebeam,Ep,Angle, XS);
                            XS_Born_XEMC = XS->gGet_XS(Ebeam, Ep, Angle, 1);//0->Rad, 1->Born 2->DIS, 3->QE
                ...
                if(!(bCal_XS&&Cross_Section->Value<-1e-33)){
                    outstring=Form("%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e%*e",
