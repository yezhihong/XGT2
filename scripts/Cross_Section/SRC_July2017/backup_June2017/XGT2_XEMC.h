...
class Get_XS
{
	public:
		...
		void Load_Table( const TString& kTarget_Name,const TString& kKin){
			...
			//while(!table.eof()){
            while(table >> aA >> aZ >> aAngle >> aEp >> aEp_Bin >> aTheta >> aTheta_Bin >> aXbj >> aQ2 >> aXS_Born >> aXS_Rad >> aTail){
...
//
		...
		//			iT, iP-1,  aEp_Table[iT][iP-1],aTheta_Table[iT][iP-1],aXS_Born_Table[iT][iP-1],aXS_Born_Table[iT][iP])<<endl;
            ...
            if(kEp<aEp_Table[iT][iP-1] || kEp>aEp_Table[iT][iP]){
                cerr<<Form("--- ERROR, Finding Ep incorrectly when reading the XS tables (XGT2_XEMC.h, #line183, Ep=%6.4f~[%d, %6.4f,%6.4f]) ", kEp, iP, aEp_Table[iT][iP-1],  aEp_Table[iT][iP])<<endl;
                outlog<<Form("--- ERROR, Finding Ep incorrectly when reading the XS tables (XGT2_XEMC.h, #line183, Ep=%6.4f~[%d, %6.4f,%6.4f]) ", kEp, iP, aEp_Table[iT][iP-1],aEp_Table[iT][iP])<<endl;
                //return -1000;
                exit(-100);
            }
            if(abs(kTheta-aTheta_Table[iT][iP])>fTheta_Bin_Size*0.5){
                cerr<<Form("--- ERROR, Finding Angle incorrectly when reading the XS tables (XGT2_XEMC.h, #line189, T=%6.4f~[%d, %6.4f,%6.4f,%6.4f]) ", kTheta, iT, aTheta_Table[iT-1][iP], aTheta_Table[iT][iP], aTheta_Table[iT+1][iP])<<endl;
                outlog<<Form("--- ERROR, Finding Angle incorrectly when reading the XS tables (XGT2_XEMC.h, #line189, T=%6.4f~[%d, %6.4f,%6.4f,%6.4f]) ", kTheta, iT, aTheta_Table[iT-1][iP], aTheta_Table[iT][iP], aTheta_Table[iT+1][iP])<<endl;
                //return -2000;
                exit(-110);
            }
