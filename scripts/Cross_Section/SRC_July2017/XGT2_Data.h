	...
	for ( unsigned int i=0; i<aRunNoChain.size(); i++ )
	{
		...
		if ( gSystem->FindFile(ROOTFILES_DIR.Data(),File_Form) )
		{
			...
			if ( aIsExtTgt ){
                ...
                if(aArm=="L")
					aEX_Ph = Form("(ExTgt%s.ph-%5.4f)",aArm.Data(),Phi_Offset_L);//Add a Phi_tg offset here, ZYe 08/07/2014
				else 
					aEX_Ph = Form("(ExTgt%s.ph-%5.4f)",aArm.Data(),Phi_Offset_R);//Add a Phi_tg offset here, ZYe 08/07/2014
				...
				if(aArm=="L")
					aEX_Ph = Form("(%s.gold.ph-%5.4f)",aArm.Data(),Phi_Offset_L);//Add a Phi_tg offset here, ZYe 08/07/2014
				else
					aEX_Ph = Form("(%s.gold.ph-%5.4f)",aArm.Data(),Phi_Offset_R);//Add a Phi_tg offset here, ZYe 08/07/2014
				aEX_SA = Form("acos((%f-%s*%f)/(sqrt(1.0+pow(%s,2)+pow(%s,2))))",aCosA,aEX_Ph.Data(),aSinA,aEX_Th.Data(),aEX_Ph.Data());
				aEX_SinSQ = Form("pow(sin(%s/2.0),2)",aEX_SA.Data()); //Define Sin(Theta/2.)^2
...
XGT2_VAR* gGet_Nf_AL(const TString& aKin, const TString& aArm, const Double_t aBin, const Double_t aBin_Size, const TString& aElectronCuts, const TString& aBin_Variable,const bool aIsExtTgt){
    ...
    Double_t MASS_AL = gGet_Target_Mass_A("Al27");
	...
	for ( unsigned int i=0; i<aRunNoChain.size(); i++ )
	{
		...
		if ( gSystem->FindFile(ROOTFILES_DIR.Data(),File_Form) )
		{
			...
			if ( aIsExtTgt ){
				...
				aEX_Th = Form("ExTgt%s.th",aArm.Data());
                if(aArm=="L")
					aEX_Ph = Form("(ExTgt%s.ph-%5.4f)",aArm.Data(),Phi_Offset_L);//Add an Phi_TG_Offset here, for L_HRS only for now
				else
					aEX_Ph = Form("(ExTgt%s.ph-%5.4f)",aArm.Data(),Phi_Offset_R);//Add an Phi_TG_Offset here, for L_HRS only for now
				aEX_SA = Form("acos((%f-%s*%f)/sqrt(1.0+pow(%s,2)+pow(%s,2)))",aCosA,aEX_Ph.Data(),aSinA,aEX_Th.Data(),aEX_Ph.Data());
				aEX_SinSQ = Form("pow(sin(%s/2.0),2)",aEX_SA.Data()); //Define Sin(Theta/2.)^2
				...
				aEX_Nu= Form("(%7.5f-%7.5f*(1.0+%s))",aE0,aP0,aEX_Dp.Data());
				aEX_Q2 = Form("4.0*%7.5f*(%7.5f*(1.0+%s))*%s",aE0,aP0,aEX_Dp.Data(),aEX_SinSQ.Data());
				aEX_Xbj=Form("%s/(2.0*%8.6f*%s)",aEX_Q2.Data(),MASS_AL,aEX_Nu.Data());//use the per-nucleon target mass for x, 07/30/2015
				//aEX_Xbj=Form("ExtEK%s.x_bj",aArm.Data());
			}
			else{
				aEX_Dp =Form("%s.gold.dp",aArm.Data());
				...
				aEX_Th = Form("%s.gold.th",aArm.Data());
				if(aArm=="L")
					aEX_Ph = Form("(%s.gold.ph-%5.4f)",aArm.Data(), Phi_Offset_L);//Add an Phi_TG_Offset here, for L_HRS only for now
				else
					aEX_Ph = Form("(%s.gold.ph-%5.4f)",aArm.Data(), Phi_Offset_R);//Add an Phi_TG_Offset here, for L_HRS only for now
				aEX_SA = Form("acos((%f-%s*%f)/(sqrt(1.0+pow(%s,2)+pow(%s,2))))",aCosA,aEX_Ph.Data(),aSinA,aEX_Th.Data(),aEX_Ph.Data());
				aEX_SinSQ = Form("pow(sin(%s/2.0),2)",aEX_SA.Data()); //Define Sin(Theta/2.)^2
