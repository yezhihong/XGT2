...
//After Oct 2016, extended the Ep ranges
const double Ep_Min_21 = 2.700, Ep_Max_21 = 3.300;
const double Ep_Min_23 = 2.650, Ep_Max_23 = 3.300;
const double Ep_Min_25 = 2.300, Ep_Max_25 = 3.300;
const double Ep_Min_28 = 2.500, Ep_Max_28 = 3.100;
class Get_XS
{
	public:
		Get_XS(){
		...
		void Load_Table( const TString& kTarget_Name,const TString& kKin){
			...
			Double_t aAngle,aEp, aTheta, aXS_Born, aXS_QE, aXS_DIS, aXS_Rad,aTail;
            Double_t aXbj, aQ2;
			...
			//while(!table.eof()){
            while(table >> aA >> aZ >> aAngle >> aEp >> aEp_Bin >> aTheta >> aTheta_Bin >> aQ2 >>aXbj >>  aXS_Born >> aXS_QE >> aXS_DIS >> aXS_Rad >> aTail){
                //table >> aA >> aZ >> aAngle >> aEp >> aEp_Bin >> aTheta >> aTheta_Bin >> aXS_Born >> aXS_Rad >> aTail;
                //After Jun20 2017, add x and Q2 values after aTheta_Bin
				//table >> aA >> aZ >> aAngle >> aEp >> aEp_Bin >> aTheta >> aTheta_Bin >> aXbj >> aQ2 >> aXS_Born >> aXS_Rad >> aTail;
                //After July12 2017, add XS_QE and XS_DIS after XS_Born
				//table >> aA >> aZ >> aAngle >> aEp >> aEp_Bin >> aTheta >> aTheta_Bin >> aXbj >> aQ2 >> aXS_Born >> aXS_QE >> aXS_DIS >> aXS_Rad >> aTail;
				...
				if(0){//if(0) to turn off
					...
					double aQ2_tmp = 4.0*aE0 * aEp * pow( sin(aTheta*TMath::DegToRad()/2.) ,2);
					double aXbj_tmp = aQ2_tmp / (2.*TARGET_MASS_A*(aE0-aEp));
				...
				aXbj_Table[aTheta_Bin][aEp_Bin] = aXbj / 0.938272; 	
				aQ2_Table[aTheta_Bin][aEp_Bin] = aQ2; 	
				aXS_Born_Table[aTheta_Bin][aEp_Bin] = aXS_Born; 	
				aXS_Rad_Table[aTheta_Bin][aEp_Bin] = aXS_Rad; 	
				aXS_DIS_Table[aTheta_Bin][aEp_Bin] = aXS_DIS; 	
				aXS_QE_Table[aTheta_Bin][aEp_Bin] = aXS_QE; 	
			...
			cerr<<" --- Done! Reading total lines = "<<aCount<<endl;
		}
		/*}}}*/
		
		/*inline double gGet_XS( const double kE0, const double kEp, const double kTheta){{{*/
		inline double gGet_XS( const double kE0, const double kEp, const double kTheta, const int kFlag=0){
			...
			//cerr<<Form("  For Theta=%f, Ep=%f, Bin_Theta=%d(%f), Bin_Ep=%d(%f)", kTheta, kEp, iT, aTheta_Table[iT][iP],iP, aEp_Table[iT][iP])<<endl;
//
			Double_t aDelta_Ep = (aEp_Table[iT][iP-1]-aEp_Table[iT][iP]);
			Double_t aXS_Born = 0.0, aXS_DIS = 0.0, aXS_QE = 0.0, aXS_Rad = 0.0;

            if(kEp<aEp_Table[iT][iP-1] || kEp>aEp_Table[iT][iP]){
                cerr<<Form("--- ERROR, Finding Ep incorrectly when reading the XS tables (XGT2_XEMC.h, #line143, Ep=%6.4f~[%d, %6.4f,%6.4f], Theta=%6.4f~[%d, %6.4f]) ", 
                        kEp, iP, aEp_Table[iT][iP-1],  aEp_Table[iT][iP], kTheta, iT, aTheta_Table[iT][iP])<<endl;
                outlog<<Form("--- ERROR, Finding Ep incorrectly when reading the XS tables (XGT2_XEMC.h, #line143, Ep=%6.4f~[%d, %6.4f,%6.4f]) ", kEp, iP, aEp_Table[iT][iP-1],aEp_Table[iT][iP])<<endl;
                //return -1000;
                exit(-100);
            }
            if(abs(kTheta-aTheta_Table[iT][iP])>fTheta_Bin_Size*0.5){
                cerr<<Form("--- ERROR, Finding Angle incorrectly when reading the XS tables (XGT2_XEMC.h, #line149, T=%6.4f~[%d, %6.4f,%6.4f,%6.4f]) ", kTheta, iT, aTheta_Table[iT-1][iP], aTheta_Table[iT][iP], aTheta_Table[iT+1][iP])<<endl;
			...
			Double_t aDelta_XS  = 0.0;
            if(fabs(aDelta_Ep)>1e-16&&aXS_Rad_Table[iT][iP]>1e-16&&aXS_Rad_Table[iT][iP-1]>1e-16){
                aDelta_XS = (aXS_Born_Table[iT][iP-1]-aXS_Born_Table[iT][iP]);
				aXS_Born  = aXS_Born_Table[iT][iP-1] - (aEp_Table[iT][iP-1]-kEp)/aDelta_Ep*aDelta_XS;

                aDelta_XS = (aXS_DIS_Table[iT][iP-1]-aXS_DIS_Table[iT][iP]);
				aXS_DIS  = aXS_DIS_Table[iT][iP-1] - (aEp_Table[iT][iP-1]-kEp)/aDelta_Ep*aDelta_XS;

                aDelta_XS = (aXS_QE_Table[iT][iP-1]-aXS_QE_Table[iT][iP]);
				aXS_QE  = aXS_QE_Table[iT][iP-1] - (aEp_Table[iT][iP-1]-kEp)/aDelta_Ep*aDelta_XS;

                aDelta_XS = (aXS_Rad_Table[iT][iP-1]-aXS_Rad_Table[iT][iP]);
            ...
            /*    double aQ2_tmp = 4.0*kE0 * kEp * pow( sin(aTheta_Table[iT][iP-1]*TMath::DegToRad()/2.) ,2);
                  double aXbj_tmp = aQ2_tmp / (2.*TARGET_MASS_A*(kE0-kEp));

                  if((aXbj_tmp > aXbj_Table[iT][iP-1])||(aXbj_tmp < aXbj_Table[iT][iP])){*/
            //cerr<<Form("--- ERROR, Finding xB incorrectly when reading the XS tables (XGT2_XEMC.h, #line169, x=%6.4f~[iT=%d, iP=%d, %6.4f,%6.4f]) ", 
            //aXbj_tmp, iT, iP,  aXbj_Table[iT][iP-1], aXbj_Table[iT][iP])<<endl;
            //outlog<<Form("--- ERROR, Finding xB incorrectly when reading the XS tables (XGT2_XEMC.h, #line169, x=%6.4f~[iT=%d, iP=%d, %6.4f,%6.4f]) ", 
            //aXbj_tmp, iT, iP,  aXbj_Table[iT][iP-1], aXbj_Table[iT][iP])<<endl;

            //cerr<<Form("--- E0 = %f,  Ep = %f, Theta = %f, mass =%f, Q2 = %f, x = %f", kE0, kEp, kTheta, TARGET_MASS_A, aQ2_tmp, aXbj_tmp)<<endl;

            /*}*/

            if(kFlag == 0) return aXS_Rad;
            if(kFlag == 1) return aXS_Born;
            if(kFlag == 2) return aXS_DIS;
            if(kFlag == 3) return aXS_QE;
            else return 0;
		}
		/*}}}*/
	...
	private:
		...
		double aQ2_Table[1000][1000]; 
		double aXbj_Table[1000][1000]; 
		...
		double aXS_DIS_Table[1000][1000];
		double aXS_QE_Table[1000][1000];
