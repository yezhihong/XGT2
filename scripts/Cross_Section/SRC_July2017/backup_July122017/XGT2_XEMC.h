...
class Get_XS
{
	public:
		...
		void Load_Table( const TString& kTarget_Name,const TString& kKin){
			...
			//while(!table.eof()){
            while(table >> aA >> aZ >> aAngle >> aEp >> aEp_Bin >> aTheta >> aTheta_Bin >> aXbj >> aQ2 >> aXS_Born >> aXS_QE >> aXS_DIS >> aXS_Rad >> aTail){
				...
				if(0){//if(0) to turn off
					...
					double aQ2_tmp = aE0 * aEp * pow( sin(aTheta*TMath::Pi()/180./2.) ,2);
				...
				aXbj_Table[aTheta_Bin][aEp_Bin] = aXbj; 	
...
//
            ...
            double aQ2_tmp = kE0 * kEp * pow( sin(kTheta*TMath::Pi()/180./2.) ,2);
