...
*/
	...
	for(int i=0;i<aNevt;i++){
		...
		if(fabs(x_fp)       <= aAccCut[0] &&
			...
			aCS = aXS->gGet_XS(E_s,E_p,Theta,0); //CS for this event
	...
	if(aBin_Variable=="Ep"){
		...
		for(int i=0;i<aNevt;i++){
			...
			if(fabs(x_fp)       < aAccCut[0] &&
				...
				aCS = aXS->gGet_XS(E_s,E_p_new,kTheta, 0) * gGet_Rad_Corr_Bump(aTarget.Data(),reactz_rec); //CS for this event
				aCS_Center = aXS->gGet_XS(E_s,aBin,kTheta, 0) * gGet_Rad_Corr_Bump(aTarget.Data(),reactz_rec); //CS for this event
    ...
    /*Xbj Bining{{{*/
		...
		for(int i=0;i<aNevt;i++){
			...
			if(fabs(x_fp)       < aAccCut[0] &&
				...
				//kTheta =  Angle_rec / TMath::DegToRad(); 
				kTheta =  Angle_new / TMath::DegToRad(); 
				//aCS = aXS->gGet_XS(E_s,E_p,kTheta, 0) * gGet_Rad_Corr_Bump(aTarget.Data(),reactz_rec); //CS for this event
				aCS = aXS->gGet_XS(E_s,E_p_new,kTheta, 0) * gGet_Rad_Corr_Bump(aTarget.Data(),reactz_rec); //CS for this event
				aCS_Center = aXS->gGet_XS(E_s,aEp_Bin,kTheta, 0) * gGet_Rad_Corr_Bump(aTarget.Data(),reactz_rec); //CS for this event
