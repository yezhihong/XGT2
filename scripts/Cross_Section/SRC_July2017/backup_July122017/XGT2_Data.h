	...
	for ( unsigned int i=0; i<aRunNoChain.size(); i++ )
	{
		...
		if ( gSystem->FindFile(ROOTFILES_DIR.Data(),File_Form) )
		{
			...
			if ( aIsExtTgt ){
                ...
                if(aArm=="L")
					aEX_Ph = Form("(ExTgt%s.ph-Phi_Offset_L)",aArm.Data());//Add a Phi_tg offset here, ZYe 08/07/2014
				else 
					aEX_Ph = Form("(ExTgt%s.ph+Phi_Offset_R)",aArm.Data());//Add a Phi_tg offset here, ZYe 08/07/2014
				...
				if(aArm=="L")
					aEX_Ph = Form("(%s.gold.ph-Phi_Offset_L)",aArm.Data());//Add a Phi_tg offset here, ZYe 08/07/2014
				else
					aEX_Ph = Form("(%s.gold.ph+Phi_Offset_R)",aArm.Data());//Add a Phi_tg offset here, ZYe 08/07/2014
				aEX_SA = Form("acos((%f-%s*%f)/(sqrt(1.0+pow(%s,2)+pow(%s,2))))",aCosA,aEX_Ph.Data(),aSinA,aEX_Th.Data(),aEX_Ph.Data());
				aEX_SinSQ = Form("pow(sin(%s/2.0),2)",aEX_SA.Data()); //Define Sin(Theta/2.)^2
	...
	for ( unsigned int i=0; i<aRunNoChain.size(); i++ )
	{
		...
		if ( gSystem->FindFile(ROOTFILES_DIR.Data(),File_Form) )
		{
			...
			if ( aIsExtTgt ){
				...
				aEX_Th = Form("ExTgt%s.th",aArm.Data());
                if(aArm=="L")
					aEX_Ph = Form("(ExTgt%s.ph-Phi_Offset_L)",aArm.Data());//Add an Phi_TG_Offset here, for L_HRS only for now
				else
					aEX_Ph = Form("(ExTgt%s.ph+Phi_Offset_R)",aArm.Data());//Add an Phi_TG_Offset here, for L_HRS only for now
				aEX_SA = Form("acos((%f-%s*%f)/sqrt(1.0+pow(%s,2)+pow(%s,2)))",aCosA,aEX_Ph.Data(),aSinA,aEX_Th.Data(),aEX_Ph.Data());
				aEX_SinSQ = Form("pow(sin(%s/2.0),2)",aEX_SA.Data()); //Define Sin(Theta/2.)^2
				...
				aEX_Th = Form("%s.gold.th",aArm.Data());
				if(aArm=="L")
					aEX_Ph = Form("(%s.gold.ph-Phi_Offset_L)",aArm.Data());//Add an Phi_TG_Offset here, for L_HRS only for now
				else
					aEX_Ph = Form("(%s.gold.ph+Phi_Offset_R)",aArm.Data());//Add an Phi_TG_Offset here, for L_HRS only for now
				aEX_SA = Form("acos((%f-%s*%f)/(sqrt(1.0+pow(%s,2)+pow(%s,2))))",aCosA,aEX_Ph.Data(),aSinA,aEX_Th.Data(),aEX_Ph.Data());
				aEX_SinSQ = Form("pow(sin(%s/2.0),2)",aEX_SA.Data()); //Define Sin(Theta/2.)^2
