...
gStyle->SetTitleFillColor(-1); 
	...
	while ( run_1>>XS>>XS_Stat_Err>>XS_Sys_Err>>xbj>>angle>>Ep>>nu>>Q2>>Ei>>p>>Livetime>>Ntg>>Ni>>Nf_EX>>Nf_SAMC>>Nf_SAMC_Gen>>Yield_EX>>Y_EX_Stat_Err>>Y_EX_Sys_Err>>Yield_MC>>Y_MC_Stat_Err>>Y_MC_Sys_Err )
	{
		if ( XS>1e-33)
		{
			...
			pEp1[Count1] = Ep;    pEp_Err1[Count1] = 1e-33;
		...
		if ( XS>1e-33)
			{
				...
				pEp2[Count2] = Ep;    pEp_Err2[Count2] = 1e-33;
...
//			bMore1 = kFALSE;
		...
		if ( XS>1e-33)
			{
				...
				pEp3[Count3] = Ep;    pEp_Err3[Count3] = 1e-33;
	...
	if(bMore2){
		...
		while ( run_4>>XS>>XS_Stat_Err>>XS_Sys_Err>>xbj>>angle>>Ep>>nu>>Q2>>Ei>>p>>Livetime>>Ntg>>Ni>>Nf_EX>>Nf_SAMC>>Nf_SAMC_Gen>>Yield_EX>>Y_EX_Stat_Err>>Y_EX_Sys_Err>>Yield_MC>>Y_MC_Stat_Err>>Y_MC_Sys_Err )
		{
			if ( XS>1e-33)
			{
				...
				pEp4[Count4] = Ep;    pEp_Err4[Count4] = 1e-33;
	...
	l1->Draw();
	c1->Print(Form("%s_XS_Kin%s.pdf",Target.Data(),Kin.Data()));
	c1->Print(Form("%s_XS_Kin%s.png",Target.Data(),Kin.Data()));
	/*}}}*/
	...
	l2->Draw();
	c2->Print(Form("%s_Yield_Kin%s.pdf",Target.Data(),Kin.Data()));
	c2->Print(Form("%s_Yield_Kin%s.png",Target.Data(),Kin.Data()));
	/*}}}*/
	...
	mg3->Draw("P");
	l3->Draw();
	c3->Print(Form("%s_YR_Kin%s.pdf",Target.Data(),Kin.Data()));
	c3->Print(Form("%s_YR_Kin%s.png",Target.Data(),Kin.Data()));
	/*}}}*/
