...
void Merge(const TString& aInputFileName, double* aXbj, double* aR_EX, double* aR_EX_Err, double* aR_MC, int* aN){
    ...
    /*Pre-define{{{*/	
    int aline = 0;
    TString aInputFilePath = "/Users/yez/work/xgt2/Xbj_Cross_Section/results/";
    ...
    Double_t x1,x2,ep1,ratio,ratio_mc,err, syst_err, stat_err, ltqe_err;
    Double_t X_Min = 10, X_Max = -10, R_Min = 1000.0, R_Max = -1000.0;
    int i=0,j=0; 
    /*Ratio: HRS-L{{{*/  
    ofstream outfileL(Form("./%s/XS_%s_%s_%s_%s_%s_LHRS.dat",OutFile_Dir.Data(), Kin0.Data(),Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
    outfileL << Form("%7s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s", 
            "Xbj","Ratio","R_TotalErr","R_StatErr(rltv)","R_LTQEErr(rltv)", "R_SystErr(rltv)",
            "XS1","XS1_StatErr(rltv)","XS1_LTQEErr(rltv)","XS1_SystErr(rltv)",
            "XS2","XS2_StatErr(rltv)","XS2_LTQEErr(rltv)","XS2_SystErr(rltv)", "R_MC", "XS1_MC", "XS2_MC" )<<endl;

    vector<Double_t> vEp_L, vXbj_L, vXbj_Err_L,vR_L, vR_Err_L,vR_StatErr_L, vR_LTQEErr_L, vR_SystErr_L, vR_MC_L;
    vector<Double_t> vXS1_L, vXS1_StatErr_L, vXS1_LTQEErr_L, vXS1_SystErr_L, vXS1_MC_L;
    vector<Double_t> vXS2_L, vXS2_StatErr_L, vXS2_LTQEErr_L, vXS2_SystErr_L, vXS2_MC_L;
    int Count_L=0;
    for( i=0;i<Count_T1L;i++){
        x1 = Xbj_T1L[i];
        ep1 = Ep_T1L[i];
        for( j=0;j<Count_T2L;j++){
            x2 = Xbj_T2L[j];
            if(x1>=0.5&&x2>=0.5&&abs(x1-x2)<0.0001){
                //Ca48 actually have 90% of Ca48+10% of Ca40
                if(Target1=="Ca48"&&Target2=="Ca40"){
                    XS_T1L[i] = ((XS_T1L[i]-0.1*XS_T2L[j])/0.9);
                }
                if(Target1=="Ca40"&&Target2=="Ca48"){
                    XS_T2L[j] = ((XS_T2L[j]-0.1*XS_T1L[i])/0.9);
                }

                ratio = factor*XS_T1L[i]/XS_T2L[j];
                //stat_err = 1./sqrt(1./pow(XS_StatErr_T1L[i]/XS_T1L[i],2)+ 1./pow(XS_StatErr_T2L[j]/XS_T2L[j],2)); //only stat
                stat_err = sqrt(pow(XS_StatErr_T1L[i]/XS_T1L[i],2)+pow(XS_StatErr_T2L[j]/XS_T2L[j],2)); //LT_stat++Ne_stat
                ltqe_err = sqrt(pow(XS_SystErr_T1L[i]/XS_T1L[i],2)+pow(XS_SystErr_T2L[j]/XS_T2L[j],2)); //LT_sys++Ne_sys
                syst_err = Ratio_Syst_Err;
                err = sqrt( stat_err*stat_err  + ltqe_err*ltqe_err+ syst_err*syst_err );

                ratio_mc = factor*XS_MC_T1L[i]/XS_MC_T2L[j];
                if(ratio>1e-30 && ratio<1000.&& (err/ratio)<1.0){
                    vEp_L.push_back(ep1);
                    vXbj_L.push_back(x1);
                    vXbj_Err_L.push_back(0.0);
                    vR_L.push_back(ratio);
                    vR_MC_L.push_back(ratio_mc);
                    vR_Err_L.push_back(ratio* err);
                    vR_StatErr_L.push_back(stat_err);//keep the normalized error outside
                    vR_LTQEErr_L.push_back(ltqe_err);//keep the normalized error outside
                    vR_SystErr_L.push_back(syst_err);//keep the normalized error outside

                    vXS1_L.push_back(XS_T1L[i]);
                    vXS1_StatErr_L.push_back(XS_StatErr_T1L[i]/XS_T1L[i]);
                    vXS1_LTQEErr_L.push_back(XS_SystErr_T1L[i]/XS_T1L[i]);
                    vXS1_SystErr_L.push_back(T1_Syst_Err_Com);
                    vXS1_MC_L.push_back(XS_MC_T1L[i]);

                    vXS2_L.push_back(XS_T2L[j]);
                    vXS2_StatErr_L.push_back(XS_StatErr_T2L[j]/XS_T2L[j]);
                    vXS2_LTQEErr_L.push_back(XS_SystErr_T2L[j]/XS_T2L[j]);
                    vXS2_SystErr_L.push_back(T2_Syst_Err_Com);
                    vXS2_MC_L.push_back(XS_MC_T2L[j]);

                    cerr <<Form("-----> HRS-L Found one: xbj1=%f, xbj2=%f, XS1=%f, XS2=%f,Ratio=%f, (stat_err=%e, syst_err=%e) ",
                            x1,x2,XS_T1L[i],XS_T2L[j],ratio, stat_err, syst_err)<<endl;
                    ...
                    if(X_Max < x1)
                        X_Max = x1;
                    Count_L++;
                    outfileL << Form("%7.5f %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e", 
                            x1,ratio,err, stat_err, ltqe_err, syst_err, 
                            XS_T1L[i], XS_StatErr_T1L[i]/XS_T1L[i], XS_SystErr_T1L[i]/XS_T1L[i],T1_Syst_Err_Com,
                            XS_T2L[j], XS_StatErr_T2L[j]/XS_T2L[j], XS_SystErr_T2L[j]/XS_T2L[j],T2_Syst_Err_Com,
                            ratio_mc, XS_MC_T1L[i],XS_MC_T2L[j] )<<endl;
                }
            }
        }
    }
    cerr<<"&&& Finished merging HRS-L, Count= "<<Count_L<<"&&&&&&&&& "<<endl<<endl;
    /*}}}*/

    /*Ratio: HRS-R{{{*/  
    ofstream outfileR(Form("./%s/XS_%s_%s_%s_%s_%s_RHRS.dat",OutFile_Dir.Data(), Kin0.Data(),Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
    outfileR << Form("%7s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s", 
            "Xbj","Ratio","R_TotalErr","R_StatErr(rltv)","R_LTQEErr(rltv)", "R_SystErr(rltv)",
            "XS1","XS1_StatErr(rltv)","XS1_LTQEErr(rltv)","XS1_SystErr(rltv)",
            "XS2","XS2_StatErr(rltv)","XS2_LTQEErr(rltv)","XS2_SystErr(rltv)", "R_MC", "XS1_MC", "XS2_MC" )<<endl;

    int Count_R=0;
    vector<Double_t> vEp_R, vXbj_R, vXbj_Err_R,vR_R, vR_Err_R,vR_StatErr_R,vR_LTQEErr_R, vR_SystErr_R, vR_MC_R;
    vector<Double_t> vXS1_R, vXS1_StatErr_R, vXS1_LTQEErr_R, vXS1_SystErr_R, vXS1_MC_R;
    vector<Double_t> vXS2_R, vXS2_StatErr_R, vXS2_LTQEErr_R, vXS2_SystErr_R, vXS2_MC_R;

    X_Min = 10; X_Max = -10; R_Min = 1000.0; R_Max = -1000.0;
    i=0; j=0; 
    for( i=0;i<Count_T1R;i++){
        x1 = Xbj_T1R[i];
        ep1 = Ep_T1R[i];
        for( j=0;j<Count_T2R;j++){
            x2 = Xbj_T2R[j];
            if(x1>=0.5&&x2>=0.5&&abs(x1-x2)<0.0001){
                //Ca48 actually have 90% of Ca48+10% of Ca40
                if(Target1=="Ca48"&&Target2=="Ca40"){
                    XS_T1R[i] = ((XS_T1R[i]-0.1*XS_T2R[j])/0.9);
                }
                if(Target1=="Ca40"&&Target2=="Ca48"){
                    XS_T2R[j] = ((XS_T2R[j]-0.1*XS_T1R[i])/0.9);
                }
                ratio = factor*XS_T1R[i]/XS_T2R[j];
                //actually stat_err includes both stat_err and LT_sys and Ne_sys
                //stat_err = 1./sqrt( 1./pow(XS_StatErr_T2R[j]/XS_T2R[j],2)+ 1./pow(XS_StatErr_T1R[i]/XS_T1R[i],2));//only stat+
                stat_err = sqrt(pow(XS_StatErr_T2R[j]/XS_T2R[j],2) +pow(XS_StatErr_T1R[i]/XS_T1R[i],2));//only LT_stat+Ne_stat
                ltqe_err = sqrt(pow(XS_SystErr_T2R[j]/XS_T2R[j],2) +pow(XS_SystErr_T1R[i]/XS_T1R[i],2));//only LT_sys+Ne_sys
                syst_err =  Ratio_Syst_Err;
                err = sqrt( stat_err*stat_err + ltqe_err*ltqe_err + syst_err*syst_err );

                ratio_mc = factor*XS_MC_T1R[i]/XS_MC_T2R[j];

                if(ratio>1e-30 && ratio<1000.&& (err/ratio)<1.0){
                    vEp_R.push_back(ep1);
                    vXbj_R.push_back(x1);
                    vXbj_Err_R.push_back(0.0);
                    vR_R.push_back(ratio);
                    vR_MC_R.push_back(ratio_mc);
                    vR_Err_R.push_back(ratio* err);
                    vR_StatErr_R.push_back(stat_err);//keep the normalized error outside
                    vR_LTQEErr_R.push_back(ltqe_err);//keep the normalized error outside
                    vR_SystErr_R.push_back(syst_err);//keep the normalized error outside

                    vXS1_R.push_back(XS_T1R[i]);
                    vXS1_StatErr_R.push_back(XS_StatErr_T1R[i]/XS_T1R[i]);
                    vXS1_LTQEErr_R.push_back(XS_SystErr_T1R[i]/XS_T1R[i]);
                    vXS1_SystErr_R.push_back(T1_Syst_Err_Com);
                    vXS1_MC_R.push_back(XS_MC_T1R[i]);

                    vXS2_R.push_back(XS_T2R[j]);
                    vXS2_StatErr_R.push_back(XS_StatErr_T2R[j]/XS_T2R[j]);
                    vXS2_LTQEErr_R.push_back(XS_SystErr_T2R[j]/XS_T2R[j]);
                    vXS2_SystErr_R.push_back(T2_Syst_Err_Com);
                    vXS2_MC_R.push_back(XS_MC_T2R[j]);

                    cerr <<Form("-----> HRS-R Found one: xbj1=%f, xbj2=%f, XS1=%f, XS2=%f,Ratio=%f, (stat_err=%e, syst_err=%e) ",
                            x1,x2,XS_T1R[i],XS_T2R[j],ratio, stat_err, syst_err)<<endl;
                    ...
                    if(X_Max < x1)
                        X_Max = x1;
                    Count_R++;
                    outfileR << Form("%7.5f %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e", 
                            x1,ratio,err, stat_err, ltqe_err, syst_err, 
                            XS_T1R[i], XS_StatErr_T1R[i]/XS_T1R[i], XS_SystErr_T1R[i]/XS_T1R[i],T1_Syst_Err_Com,
                            XS_T2R[j], XS_StatErr_T2R[j]/XS_T2R[j], XS_SystErr_T2R[j]/XS_T2R[j],T2_Syst_Err_Com,
                            ratio_mc, XS_MC_T1R[i],XS_MC_T2R[j] )<<endl;
                }
            }
        }
    }
    cerr<<"&&& Finished merging HRS-R, Count= "<<Count_R<<"&&&&&&&&& "<<endl<<endl;
    /*}}}*/

    /*Add L+R Ratio{{{*/
    ofstream outfile(Form("./%s/XS_%s_%s_%s_%s_%s.dat",OutFile_Dir.Data(), Kin0.Data(),Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
    outfile << Form("%7s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s %14s", 
            "Xbj","Ratio","R_TotalErr","R_StatErr(rltv)","R_LTQEErr(rltv)","R_SystErr(rltv)",
            "XS1","XS1_StatErr(rltv)","XS1_LTQEErr(rltv)","XS1_SystErr(rltv)",
            "XS2","XS2_StatErr(rltv)","XS2_LTQEErr(rltv)","XS2_SystErr(rltv)", "R_MC", "XS1_MC", "XS2_MC" )<<endl;

    int Count=0;
    vector<double> vEp, vXbj, vXbj_Err,vR, vR_Err,vR_StatErr,vR_LTQEErr,vR_SystErr,vR_MC;
    vector<Double_t> vXS1_LR, vXS1_StatErr_LR, vXS1_LTQEErr_LR, vXS1_SystErr_LR, vXS1_MC_LR;
    vector<Double_t> vXS2_LR, vXS2_StatErr_LR, vXS2_LTQEErr_LR, vXS2_SystErr_LR, vXS2_MC_LR;

    int J_Both= 0;
    double R_Both = 0.0, R_MC_Both = 0.0, R_Err=0.0, R_StatErr=0.0, R_LTQEErr=0.0, R_SystErr=0.0;
    bool bFound = kFALSE;
    /*Staring from L-arm, try to find a match on R-arm{{{*/
    for(int i=0;i<Count_L; i++){
        bFound = kFALSE;
        R_Both = 0.0; R_Err=0.0; R_StatErr=0.0; R_SystErr=0.0; R_MC_Both = 0.0;
        for(int j=0;j<Count_R; j++){
            if( abs(vXbj_L[i] - vXbj_R[j])<0.0001 ){
                R_Both = (vR_L[i] + vR_R[j])/2.0;
                //R_StatErr = 1./sqrt( 1./pow(vR_StatErr_L[i],2) + 1./pow(vR_StatErr_R[j],2) );
                R_StatErr = sqrt( pow(vR_StatErr_L[i],2) + pow(vR_StatErr_R[j],2) );
                R_LTQEErr = sqrt( pow(vR_LTQEErr_L[i],2) + pow(vR_LTQEErr_R[j],2) );
                R_SystErr = sqrt( pow(vR_SystErr_L[i],2) + pow(vR_SystErr_R[j],2) );
                R_Err = sqrt( R_StatErr*R_StatErr + R_LTQEErr*R_LTQEErr + R_SystErr*R_SystErr );
                R_MC_Both = (vR_MC_L[i]+vR_MC_R[j])/2.0;
                
                vEp.push_back(vEp_L[i]);	
                vXbj.push_back(vXbj_L[i]);	
                vXbj_Err.push_back(0.0);
                vR.push_back(R_Both);
                vR_MC.push_back(R_MC_Both);
                vR_Err.push_back(R_Both*R_Err);
                vR_StatErr.push_back(R_StatErr);
                vR_LTQEErr.push_back(R_LTQEErr);
                vR_SystErr.push_back(R_SystErr);

                vXS1_LR.push_back((vXS1_L[i] + vXS1_R[j])/2.0);
                vXS1_MC_LR.push_back((vXS1_MC_L[i] + vXS1_MC_R[j])/2.0);
                vXS1_StatErr_LR.push_back(sqrt(pow(vXS1_StatErr_L[i],2) + pow(vXS1_StatErr_R[j],2)));
                vXS1_LTQEErr_LR.push_back(sqrt(pow(vXS1_LTQEErr_L[i],2) + pow(vXS1_LTQEErr_R[j],2)));
                vXS1_SystErr_LR.push_back(sqrt(pow(vXS1_SystErr_L[i],2) + pow(vXS1_SystErr_R[j],2)));

                vXS2_LR.push_back((vXS2_L[i] + vXS2_R[j])/2.0);
                vXS2_MC_LR.push_back((vXS2_MC_L[i] + vXS2_MC_R[j])/2.0);
                vXS2_StatErr_LR.push_back(sqrt(pow(vXS2_StatErr_L[i],2) + pow(vXS2_StatErr_R[j],2)));
                vXS2_LTQEErr_LR.push_back(sqrt(pow(vXS2_LTQEErr_L[i],2) + pow(vXS2_LTQEErr_R[j],2)));
                vXS2_SystErr_LR.push_back(sqrt(pow(vXS2_SystErr_L[i],2) + pow(vXS2_SystErr_R[j],2)));
        ...
        if(!bFound){
            R_Both = vR_L[i];
            R_MC_Both = vR_MC_L[i];
            R_StatErr = vR_StatErr_L[i];
            R_LTQEErr = vR_LTQEErr_L[i];
            R_SystErr = vR_SystErr_L[i];
            R_Err = sqrt( R_StatErr*R_StatErr+R_LTQEErr*R_LTQEErr + R_SystErr*R_SystErr );
            ...
            vXS1_LR.push_back(vXS1_L[i]);
            vXS1_MC_LR.push_back(vXS1_MC_L[i]);
            vXS1_StatErr_LR.push_back(vXS1_StatErr_L[i]);
            vXS1_LTQEErr_LR.push_back(vXS1_LTQEErr_L[i]);
            vXS1_SystErr_LR.push_back(vXS1_SystErr_L[i]);

            vXS2_LR.push_back(vXS2_L[i]);
            vXS2_MC_LR.push_back(vXS2_MC_L[i]);
            vXS2_StatErr_LR.push_back(vXS2_StatErr_L[i]);
            vXS2_LTQEErr_LR.push_back(vXS2_LTQEErr_L[i]);
            vXS2_SystErr_LR.push_back(vXS2_SystErr_L[i]);
    ...
    //Pick up leftover of the R-arm data points:/*{{{*/
    R_Both = 0.0; R_Err=0.0; R_StatErr=0.0; R_SystErr=0.0; R_MC_Both =0.0;
    for(int k = J_Both+1; k<Count_R; k++){
        R_Both = vR_R[k];
        R_MC_Both = vR_MC_R[k];
        R_StatErr = vR_StatErr_R[k];
        R_LTQEErr = vR_LTQEErr_R[k];
        R_SystErr = vR_SystErr_R[k];
        R_Err = sqrt( R_StatErr*R_StatErr+R_LTQEErr*R_LTQEErr + R_SystErr*R_SystErr );

        vEp.push_back(vEp_R[k]);	
        vXbj.push_back(vXbj_R[k]);	
        vXbj_Err.push_back(0.0);
        vR.push_back(R_Both);
        vR_MC.push_back(R_MC_Both);
        vR_Err.push_back(R_Both*R_Err);
        ...
        vXS1_LR.push_back(vXS1_R[k]);
        vXS1_MC_LR.push_back(vXS1_MC_R[k]);
        vXS1_StatErr_LR.push_back(vXS1_StatErr_R[k]);
        vXS1_LTQEErr_LR.push_back(vXS1_LTQEErr_R[k]);
        vXS1_SystErr_LR.push_back(vXS1_SystErr_R[k]);

        vXS2_LR.push_back(vXS2_R[k]);
        vXS2_MC_LR.push_back(vXS2_MC_R[k]);
        vXS2_StatErr_LR.push_back(vXS2_StatErr_R[k]);
        vXS2_LTQEErr_LR.push_back(vXS2_LTQEErr_R[k]);
        vXS2_SystErr_LR.push_back(vXS2_SystErr_R[k]);
    ...
    for(int k=0;k<vXbj.size();k++)
        outfile << Form("%7.5f %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e %14e", 
                vXbj_R[k], vR[k],vR_Err[k], vR_StatErr[k], vR_LTQEErr[k], vR_SystErr[k],
                vXS1_LR[k], vXS1_StatErr_LR[k], vXS1_LTQEErr_LR[k], vXS1_SystErr_LR[k],
                vXS2_LR[k], vXS2_StatErr_LR[k], vXS2_LTQEErr_LR[k], vXS2_SystErr_LR[k],
                vR_MC[k], vXS1_MC_LR[k],vXS2_MC_LR[k]
                )<<endl;
...
void Merge25(const TString& Target0, const TString& Target, double* aXbj, double* aR_EX, double* aR_EX_Err, double* aR_MC, int* aN){
    ...
    vector<double> vXbj1, vXS11,vR_Err1,vR_StatErr1,vR_LTQEErr1,vR_SystErr1, vXS11_StatErr, vXS11_LTQEErr,vXS11_SystErr, vXS12, vXS21_StatErr, vXS21_LTQEErr,vXS21_SystErr;
    vector<double> vXbj2, vXS21,vR_Err2,vR_StatErr2,vR_LTQEErr2,vR_SystErr2, vXS12_StatErr, vXS12_LTQEErr,vXS12_SystErr, vXS22, vXS22_StatErr, vXS22_LTQEErr,vXS22_SystErr;
    vector<double> vXS11_MC, vXS21_MC, vXS12_MC, vXS22_MC;

    /*Readin{{{*/
    double x, R, R_err, R_systerr, R_ltqeerr,R_staterr,xs1, xs1_staterr,xs1_ltqeerr, xs1_systerr,xs2, xs2_staterr,xs2_ltqeerr, xs2_systerr,R_MC, xs1_mc, xs2_mc;
    TString dum;
    infile1 >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum>>dum >>dum >>dum;
    while(infile1 >> x >> R >> R_err >> R_staterr>> R_ltqeerr>> R_systerr >> xs1>> xs1_staterr>>xs1_ltqeerr>>xs1_systerr>>xs2>> xs2_staterr>>xs2_ltqeerr>>xs2_systerr>>R_MC>>xs1_mc>>xs2_mc){

        cerr<<Form("Kin5.0: x=%6.4f,  R = %6.4f, R_Err = %6.4f", x, R, R_err)<<endl;;
        vXbj1.push_back(x);
        vR_StatErr1.push_back(R_staterr);
        vR_LTQEErr1.push_back(R_ltqeerr);
        vR_SystErr1.push_back(R_systerr);
        vR_Err1.push_back(R_err);
    ...
    infile2 >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum >>dum;
    while(infile2 >> x >> R >> R_err >> R_staterr >>R_ltqeerr>> R_systerr >> xs1>> xs1_staterr>>xs1_ltqeerr>>xs1_systerr>>xs2>> xs2_staterr>>xs2_ltqeerr>>xs2_systerr>>R_MC>>xs1_mc>>xs2_mc){
        cerr<<Form("Kin5.1: x=%6.4f,  R = %6.4f, R_Err = %6.4f", x, R, R_err)<<endl;;
        vXbj2.push_back(x);
        vR_StatErr2.push_back(R_staterr);
        vR_LTQEErr2.push_back(R_ltqeerr);
        vR_SystErr2.push_back(R_systerr);
        vR_Err2.push_back(R_err);
    ...
    /*Combinning{{{*/
    int Count=0;
    vector<double> vXbj, vR, vR_Err, vR_StatErr, vR_LTQEErr,vR_SystErr,vXS1, vXS1_StatErr, vXS1_LTQEErr,vXS1_SystErr, vXS2, vXS2_StatErr, vXS2_LTQEErr,vXS2_SystErr, vR_MC, vXS1_MC, vXS2_MC;
    Double_t x1,x2,ratio,err, syst_err, ltqe_err,stat_err, ratio_mc;
    ...
    for(unsigned int i=0;i<vXbj1.size();i++){
        ...
        for(unsigned int j=0;j<vXbj2.size();j++){
            ...
            if(x1>=0.5&&x2>=0.5&&abs(x1-x2)<0.0001){
                ...
                double xs1_stat_err = (vXS11_StatErr[i] + vXS12_StatErr[j])/2.0;
                ...
                double xs2_stat_err = (vXS21_StatErr[i] + vXS22_StatErr[j])/2.0;
                double xs2_syst_err = (vXS21_SystErr[i] + vXS22_SystErr[j])/2.0;
                double xs2_ltqe_err = (vXS21_LTQEErr[i] + vXS22_LTQEErr[j])/2.0;

                double ratio_stat_err = (vR_StatErr1[i]+vR_StatErr2[j])/2.0;
                double ratio_ltqe_err = (vR_LTQEErr1[i]+vR_LTQEErr2[j])/2.0;
                double ratio_syst_err = (vR_SystErr1[i]+vR_SystErr2[j])/2.0;
                double ratio_err = sqrt( ratio_stat_err*ratio_stat_err+ratio_ltqe_err*ratio_ltqe_err+ratio_syst_err*ratio_syst_err);

                ratio = factor00*xs1/xs2;
                ratio_mc = factor00*xs1_mc/xs2_mc;
                //actually stat_er5 includes both stat_err and LT_sys and Ne_sys
                //stat_err = 1./sqrt(1./pow( xs1_stat_err,2) + 1./pow( xs2_stat_err,2));
                stat_err = sqrt(pow( xs1_stat_err,2) + pow( xs2_stat_err,2));
                ltqe_err = sqrt(pow( xs1_ltqe_err,2) + pow( xs2_ltqe_err,2));
                syst_err = sqrt(pow( xs1_syst_err,2) + pow( xs2_syst_err,2));
                err = sqrt( stat_err*stat_err + ltqe_err*ltqe_err + syst_err*syst_err );
                
                //double err_test = sqrt( pow( xs1_stat_err,2) + pow( xs2_stat_err,2) + pow( xs1_ltqe_err,2) + pow( xs2_ltqe_err,2) + pow(ratio_syst_err,2) );
                ...
                if(ratio>1e-30 && ratio<1000.&& (err/ratio)<1.0){
                    ...
                    vR_Err.push_back(ratio*err);
                    vR_StatErr.push_back(stat_err);
                    vR_LTQEErr.push_back(ltqe_err);
                    vR_StatErr.push_back(stat_err);
                    
                    //vR_Err.push_back(ratio*ratio_err);
                    //vR_StatErr.push_back(ratio_stat_err);
                    //vR_LTQEErr.push_back(ratio_ltqe_err);
                    //vR_StatErr.push_back(ratio_stat_err);
        ...
        if(!(bFound)){
            ...
            double ratio_stat_err = vR_StatErr1[i];
            double ratio_ltqe_err = vR_LTQEErr1[i];
            double ratio_syst_err = vR_SystErr1[i];
            //double ratio_err = vR_Err1[i];
            double ratio_err = sqrt( ratio_stat_err*ratio_stat_err+ratio_ltqe_err*ratio_ltqe_err+ratio_syst_err*ratio_syst_err);
            ...
            if(ratio>1e-30 && ratio<1000.&& (err/ratio)<1.0){
                ...
                vR_StatErr.push_back(stat_err);
                vR_LTQEErr.push_back(ltqe_err);
                vR_SystErr.push_back(syst_err);

                //vR_StatErr.push_back(ratio_stat_err);
                //vR_LTQEErr.push_back(ratio_ltqe_err);
                //vR_SystErr.push_back(ratio_syst_err);
    ...
    for(unsigned int i=J2+1;i<vXbj2.size();i++){
        ...
        double ratio_stat_err = vR_StatErr2[i];
        double ratio_ltqe_err = vR_LTQEErr2[i];
        double ratio_syst_err = vR_SystErr2[i];
        double ratio_err = sqrt( ratio_stat_err*ratio_stat_err+ratio_ltqe_err*ratio_ltqe_err+ratio_syst_err*ratio_syst_err);
    ...
    if(J20>=1){
        for(unsigned int i=0;i<J20;i++){
            ...
            //double err_test = ratio * sqrt( pow( xs1_stat_err,2) + pow( xs2_stat_err,2) + pow( xs1_ltqe_err,2) + pow( xs2_ltqe_err,2) + pow(ratio_syst_err,2) );
...
void Merge212325(const TString& Target0, const TString& Target, double* aXbj, double* aR_EX, double* aR_EX_Err, double* aR_MC, int* aN){
    ...
    for(unsigned int i=0;i<vXbj1.size();i++){
        ...
        for(unsigned int j=0;j<vXbj2.size();j++){
            x2 = vXbj2[j];
            /*Combine two bins{{{*/
            if(x1>=0.5&&x2>=0.5&&abs(x1-x2)<0.001){
                if(bFirst){
                    bFirst = kFALSE;
                    J20= j;
                }
