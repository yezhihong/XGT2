#include "/Users/yez/work/Xbj_Cross_Section/SRC/XGT2_Main.h"
#include "/Users/yez/work/Xbj_Cross_Section/SRC/XGT2_XEMC.h"
#include "TLegend.h"
#include "TLine.h"
...
const double Xbj_Cut_21_2[4] = {1.65, 2.80,1.65,2.80};//3.00}; 
const double Xbj_Cut_23_1[4] = {1.30, 2.15,1.30,2.15}; 
const double Xbj_Cut_23_2[4] = {1.85, 2.90,1.85,2.90};//3.00};
...
const double Xbj_Cut_25_2[4] = {2.00, 2.90,2.00,2.90};//3.00}; 
const double Xbj_Cut_28_1[4] = {1.80, 2.90,1.80,2.90};//3.00}; 
/*
...
/*}}}*/
	...
	ex2325->SetMarkerStyle(20);
	ex2325->SetMarkerSize(1.5);
	ex2325->SetMarkerColor(1);
	ex2325->SetLineColor(1);
    ex2325->SetLineWidth(1.5);
    
    ofstream out_2325("E08014_T2325.dat");
    for(int i=0;i<N_2325;i++)
        out_2325 << Form("%3d  %5.2f  %8.4f  %10.6f", i, xbj_2325[i], R_EX_2325[i], R_EX_Err_2325[i])<<endl;
    out_2325.close();
...
/*}}}*/
    ...
    //if(Target=="He3"){
		...
		exNF->SetMarkerStyle(24);
		exNF->SetMarkerSize(1.5);
		exNF->SetMarkerColor(4);
		exNF->SetLineColor(4);
		exNF->SetLineWidth(1.2);
		c3->cd(); exNF->Draw("P");
		c31->cd(); exNF->Draw("P");
		c32->cd(); exNF->Draw("P");
		c4->cd(); exNF->Draw("P");
        ...
        ofstream out_e02019("E02019.dat");
        for(int i=0;i<N_NF;i++)
        out_e02019 << Form("%3d  %5.2f  %8.4f  %10.6f  %10.6f", i, xbj_NF[i], R_EX_NF[i], R_EX_Err_Down[i],R_EX_Err_Up[i])<<endl;
        out_e02019.close();
    }
    ...
    if(Target0=="He4"&&Target=="He3"){
		exCS->SetMarkerStyle(21);
		exCS->SetMarkerSize(1.5);
		//exCS->SetMarkerColor(33);
		exCS->SetMarkerColor(6);
		exCS->SetLineColor(6);
		...
		c32->cd(); exCS->Draw("P");
		c4->cd(); exCS->Draw("P");
		l1->AddEntry(exCS,Form("CLAS6 Data"),"p");
   
        ofstream out_clas("CLAS6.dat");
        for(int i=0;i<N_CS;i++)
            out_clas << Form("%3d  %5.2f  %8.4f  %10.6f", i, xbj_CS[i], R_EX_CS[i], R_EX_Err_CS[i])<<endl;
        out_clas.close();
 }
   ...
   /* if(!(Target=="H2")){*/
	...
	l11->AddEntry(ex2325, Form("E08014 Data (<Q^{2}>=1.7 GeV/c^{2})"),"p");
    ...
    if(Target=="He3"){
        l11->AddEntry(exNF,Form("E02019 Data (<Q^{2}>=2.7 GeV/c^{2})"),"p");
        l12->AddEntry(exNF,Form("E02019 Data (18^{#circ},Q^{2}=2.7 GeV/c^{2})"),"p");
    }
    if(Target0=="He4"&&Target=="He3"){
        l11->AddEntry(exCS,Form("CLAS6 Data (<Q^{2}>=1.6 GeV/c^{2})"),"p");
        l12->AddEntry(exCS,Form("CLAS6 Data"),"p");
	}
    ...
    c32->Print(Form("./both/%s_%s_XS_Ratio_%s_%s_212325.png",Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
    c32->Print(Form("./both/%s_%s_XS_Ratio_%s_%s_212325.pdf",Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
    ...
    if(bMC){	
        ...
        c3->cd();	mcQE->Draw("l");
        c4->cd();	mcQE->Draw("l");
        //l1->AddEntry(mcQE, Form("R_{MC} at %d^{#circ} (QE) ",25),"l");
        ...
        c32->Print(Form("./both/%s_%s_XS_Ratio_MC_%s_%s_212325.png",Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
        c32->Print(Form("./both/%s_%s_XS_Ratio_MC_%s_%s_212325.pdf",Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
...
void Merge(const TString& aInputFileName, double* aXbj, double* aR_EX, double* aR_EX_Err, double* aR_MC, int* aN){
	...
	/*Pre-define{{{*/	
	int aline = 0;
	TString aInputFilePath = "/Users/yez/work/Xbj_Cross_Section/results/";
	...
	{
		if ( xbj>=Xbj_Min && xbj<=Xbj_Max && angle<180 &&xbj>0.5&&xbj<=3.00 && XS>0.0 && sqrt(pow(XS_Syst_Err,2)+pow(XS_Stat_Err,2))/XS <0.5)
		{
...
void Merge212325(const TString& Target0, const TString& Target, double* aXbj, double* aR_EX, double* aR_EX_Err, double* aR_MC, int* aN){
	...
	ofstream outfile(Form("./both/XS_212325_%s_%s_%s_%s_Total.dat",Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
	ifstream infile1;
	infile1.open(Form("./both/XS_2325_%s_%s_%s_%s_Total.dat",Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
	ifstream infile2;
    infile2.open(Form("./both/XS_3.1_%s_%s_%s_%s.dat",Target0.Data(),Target.Data(),Com0.Data(),Com.Data()));
...
void E02019(const TString& aInputFileName, const double aTheta,double* aXbj, double* aR_EX, double* aR_EX_Err, int* aN){
	int aline = 0;
	TString aInputFilePath = "/Users/yez/work/Xbj_Cross_Section/results/Ratio/";
